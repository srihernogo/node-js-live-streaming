    ...
    checkSQL: function(req,singularType,type,primary_id) {
        return new Promise(function(resolve, reject) {
            req.getConnection(async function(err,connection){
                let sql = ""
                if(req.user){
                    sql += type+".owner_id NOT IN (SELECT resource_id from user_blocks WHERE owner_id = "+req.user.user_id+" ) "
                }
                ...
                */
                ...
                sql += 'CASE  WHEN '+type+'.owner_id = '+owner_id+' THEN true WHEN view_privacy = "onlyme" THEN false WHEN view_privacy = "everyone" THEN true '
                if(req.isview)
                ...
                if(type == "videos"){
                    ...
                    if(req.appSettings['whitelist_domain'] == 1){
                            ...
                            if(req.user){
                                ...
                                let userIP = typeof req.headers != "undefined" && req.headers['x-real-ip'] ? req.headers['x-real-ip'] : (req.socket && req.socket.remoteAddress ? req.socket.remoteAddress : "");
    ...
    check:  function(req,obj,type){
        return new Promise(function(resolve, reject) {
            req.getConnection(async function(err,connection){
                ...
                let isValid = true;
                if(obj.owner_id && req.user){
                    await globalModel.custom(req,"SELECT * FROM user_blocks WHERE owner_id = ? AND resource_id = ?",[req.user.user_id,obj.owner_id]) .then(results => {
