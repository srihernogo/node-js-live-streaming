...
const CarouselPlaylist = (props) => {
  ...
  useEffect(() => {
    if (props.playlists != state.playlists) {
      setState({ playlists: props.playlists, key: props.key + 1 });
    }
  }, [props]);
  ...
  const content = state.playlists.map((result) => {
    return (
      <div key={result.playlist_id}>
        ...
        <div className="row">
          <div className="col-sm-12">
            <div className="titleWrap">
              ...
              {props.seemore && state.playlists.length > 4 ? (
                <Link
                  href={`/playlists?${props.type ? "type" : "sort"}=${
                    props.type ? props.type : props.sort
                  }`}
                >
                  <a className="seemore_link">See more</a>
                </Link>
              ) : null}
            </div>
          </div>
