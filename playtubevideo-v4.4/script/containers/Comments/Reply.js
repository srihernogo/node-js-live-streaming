...
const [state, setState] = useReducer(
    ...
    const linkify = (inputText) => {
        ...
        return (
            <React.Fragment>
                <Header {...props} linkify={linkify} textHeading="Show replies for:" title={state.item.title} item={state.item} image={state.comments[0].type == "members" ? state.item.avtar : state.item.image} type={state.comments[0].type} />
                <Comment  {...props} id={state.id} paggingComment={state.pagging} comments={state.comments} hideTitle={true} appSettings={props.pageData.appSettings} commentType={commentType} type={state.comments[0].type} comment_item_id={state.comments[0].type == "members" ? state.item.user_id : state.item[commentType+"_id"]} />
            </React.Fragment>
        )
    }
