module.exports = {
    ...
    delete: function(data,req,res){
        return new Promise(function(resolve, reject) {
            req.getConnection(function(err,connection){
                connection.query('DELETE from playlistvideos where video_id = ? and playlist_id = ? ',[data.video_id,data.playlist_id],function(err,results,fields)
                    {
                        resolve(true)
                    }
                ) 
